...
// 👇 1. IMPORTANTE: Agregamos el hook de autenticación
...
export default function Cart({ carrito, setCarrito }) {
  ...
  const processPayment = async (e) => {
    ...
    // Si no hay usuario, mandarlo al login antes de pagar
    if (!user) {
    ...
    setIsLoading(true);
    const orderId = 'NIL-' + Date.now().toString().slice(-6);
    ...
    try {
      ...
      // 4. SEGUNDO: Enviamos los correos
      ...
      setModal({ show: true, type: 'error', title: 'Error', message: 'Hubo un problema al procesar el pedido.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    /* Mantenemos tu mismo return con el estilo de 100px para que no se encime */
          ...
          <div style={{ padding: '30px' }}>
            {carrito.length === 0 ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <Link to="/menu" style={{ color: '#b5835a' }}>Ir a comprar postres</Link>
              </div>
            ) : (
              <>
                {!mostrarPago ? (
       ...
       {modal.show && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', padding: '30px', borderRadius: '15px', textAlign: 'center', maxWidth: '400px' }}>
              ...
              <a href={modal.whatsappLink} target="_blank" style={{ display: 'block', background: '#25D366', color: 'white', padding: '12px', borderRadius: '5px', textDecoration: 'none', margin: '20px 0' }}>
